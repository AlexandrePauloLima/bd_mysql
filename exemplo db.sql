...
INSERT INTO to_produtos(nome,quantidade,preco)
VALUES ( "Sacolas CINZA", 1118,0.50);
SElECT * FROM tb_produtos;
SELECT * FROM tb_produtos ORDER BY nome;
SELECT * FROM tb_produtos ORDEr BY produto;
SELECT * FROM tb_produtos WHERE nome LIKE "ra%";
SELECT * FROM tb_produtos WHERE nome LIKE "%ra%";
SELECT * FROM tb_produtos WHERE nome LIKE "%ra";

SELECT COUNT(*) FROM tb_produtos;
SELECT COUNT (categoria_id) FROM tb_produto ;
SELECT SUM (preco) FROM tb_produtos;


-- valor maximo na lista
...
FROM tb_produtos INNER JOIN tb_catergorias
ON tb_produtos.categoria_id = tb_categorias.id;

SELECT nome, preco, quantidade, tb_categorias.descricao
FROM tb_produtos LEFT JOIN tb_catergorias
ON tb_produtos.categoria_id = tb_categorias.id;

SELECT nome, preco, quantidade, tb_categorias.descricao
FROM tb_produtos RIGHT JOIN tb_catergorias
ON tb_produtos.categoria_id = tb_categorias.id;
...
SELECT * FROM tb_produtos WHERE preco > 5.00 AND categoria_id = 2 LIMIT 0, 1000
